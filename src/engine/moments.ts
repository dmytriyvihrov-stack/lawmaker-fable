 ...
 *
 * One of them, never four, so the map is never a row of buttons and a year is
 * never the same year twice. Which one comes off the seed and the turn, so a
 * reign reloaded from a save finds the same one where it left it.
 *
 ...
 */
export const MOMENTS_PER_YEAR = 1;
...
export function momentsOfYear(s: GameState, season: Season): Moment[] {
  const fit = MOMENTS.filter(
    (m) =>
      (m.seasons === undefined || m.seasons.includes(season)) &&
      (m.needsFlag === undefined || s.flags.includes(m.needsFlag)),
  );
