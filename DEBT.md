...
Written against `a3fc486` (V54). Everything numeric here came out of this repo:
...
> **Before editing anything from this file:** step 1 of `deploy.ps1` mirrors
> `..\Lawmaker Dilemmas\{src,tests,tools}` over this repo with `robocopy /MIR`.
> Fixes made here to `src/` are destroyed by the next deploy. Carry them to the
> desk. This file lives at the root, which the mirror does not touch, so it
> survives.
